 ...
 * @author c2learn
 */
@Path("4scribes/typical")
@Produces("application/json")
public class FourSTypical {
	...
	protected int getMinDistanceIndex(ArrayList<ElementCard> cardSet){
		...
		for (int card_count = 0; card_count < cardSet.size(); card_count++) {
			if (cardSet.get(max_diff_index).getID() != cardSet.get(card_count).getID()) {
				double value = SemanticDiffTypicalTable.getInstance().getTypicalTomValueAndTimesPlayed(cardSet.get(card_count));
				//System.out.println("VALUE IS = " + value + " CURRENT MIN = " + current_min);
    ...
    /**
     * Retrieves representation of an instance of com.idg.idgweb.scribes.FourSTypical
    ...
    public ArrayList<ArrayList<ElementCard>> getJson(@PathParam("players") String num_players, @PathParam("cards") String cards) {
        ...
        while (numTotal_cards > 0) {
			int max_diff_index = getMinDistanceIndex(nonPersonalities_available);
