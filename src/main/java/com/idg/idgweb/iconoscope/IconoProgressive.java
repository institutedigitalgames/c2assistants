 ...
 * @author c2learn
 */
@Path("icono/progressive")
@Produces("application/json")
public class IconoProgressive {
    /**
     * Creates a new instance of FourSChaotic
     */
    public IconoProgressive() {
    }
    ...
    public ArrayList<Canvas> getJson(String inputJson) {
        ...
        //System.out.println("Progressive Petra Initialization...");
		...
		//System.out.println("Loading typical set from "+pc.getString("4scribes_decks") + targetWord+".xml");
		File f = new File(pc.getString("4scribes_decks") + File.separator + targetWord+".xml");
        if(f.exists()){ 
			//System.out.println("File "+targetWord+".xml"+" FOUND!"); 
		} else {
			Canvas toSave = SaveRandomConcepts.getRandomConcept();
			Easy.save(toSave,pc.getString("4scribes_decks") + File.separator + targetWord+".xml");
			//System.out.println("New "+parsed.getApp().SelectedWord+".xml"+" CREATED!");
		}
		
		Canvas typicalExample = null;
		try {
			typicalExample = (Canvas)(Easy.load(pc.getString("4scribes_decks") + File.separator + targetWord+".xml"));
		} catch(Exception e){ System.out.println("typical example for "+targetWord+" not found!"); }
		//System.out.println("Progressive Petra found the typical concept "+typicalExample);
